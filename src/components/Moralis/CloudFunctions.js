...
Moralis.Cloud.define("loadInboxMessage", async (request) => {
...
Moralis.Cloud.define("loadInboxProfiles", async (request) => {
const logger = Moralis.Cloud.getLogger();
...
const query = relation.query({useMasterKey:true});
  ...
  query.equalTo('requestAccepted', true)  	
  query.select('user1.username', 'user1.profilePic', 'date', 'projectName');
   query.descending('createdAt');
  const queryResults = await query.find({useMasterKey:true});
const query2 = relation.query({useMasterKey:true});
  query2.notEqualTo('user2', user)
  query2.equalTo('requestAccepted', true)  	
  query2.select('user2.username', 'user2.profilePic', 'date', 'projectName');
   query2.descending('createdAt');
  const queryResults2 = await query2.find({useMasterKey:true});
  const results = [];
  for (let i = 0; i < queryResults.length; ++i) {
  if(typeof queryResults[i] === 'undefined') {
     continue;
  } else {
    let profilePic;
  ...
  if(typeof queryResults2[i] === 'undefined') {
     continue;
  } else {
    let profilePic;
    if (typeof queryResults2[i].attributes.user2.attributes.profilePic === 'undefined') {
           profilePic = "https://i.ibb.co/mvFxwhM/default-Profile.png";
    } else {
       profilePic = queryResults2[i].attributes.user2.attributes.profilePic._url;
    }
    results.push({
      "username": queryResults2[i].attributes.user2.attributes.username,
      "date": queryResults2[i].attributes.date,
      "profilePic": profilePic,
      "projectName": queryResults2[i].attributes.projectName,
      "chatId" : queryResults2[i].id,
    });        
}};
...
const query = relation.query({useMasterKey:true});
  ...
  const query2 = relation.query({useMasterKey:true});
  query2.equalTo('user1', user)
  query2.equalTo('notifyUser2', true)  	
  query2.select('user2.username', 'user2.profilePic', 'date', 'projectName');
   query2.descending('createdAt');
  const queryResults2 = await query2.find({useMasterKey:true});
...
const Profile = Moralis.Object.extend("User");
 ...
 messages.set('parent', conversation);
  await messages.save();    
  return messages;
});
...
const queryResults = await query.find({useMasterKey:true}); 
logger.info(queryResults[0].title);
...
Moralis.Cloud.define("renderProjects", async (request) => {
  const skipAmount = (request.params.skipAmount)*(9);
const query = new Moralis.Query("Projects");
  query.select('creatorProfilePic', 'title','projectPhoto', 'summary','isVerified', 'date','description', 'creator.username', 'creator.profilePic', 'createdAt');
  query.descending('createdAt');    
...
Moralis.Cloud.define("renderMyProjects", async (request) => {
  const rowsSkipped = ( request.params.page * 6);
  const currentUser = request.user;
...
Moralis.Cloud.define("renderUserProjects", async (request) => {
    const skipAmount = (request.params.skipAmount)*(3);
...
const skipAmount = (request.params.pageNum * 9);
